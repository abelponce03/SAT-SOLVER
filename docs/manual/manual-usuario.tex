...
\subsection{Ruptura de simetrías dentro del binario}
Con \texttt{./scripts/build.sh --dir=build-symm --symmetry}, satsuma (incluido
en \texttt{solver/satsuma}, sin modificar) se compila dentro de Kissat y queda
un \textbf{único binario}, \texttt{solver/kissat/build-symm/kissat}. Hace
falta un compilador de C++20. La ruptura se activa con la opción
\texttt{--symmetry} del propio binario (ADR-0007):
\begin{lstlisting}[language=bash]
solver/kissat/build-symm/kissat --symmetry formula.cnf.xz prueba.out
\end{lstlisting}
Hace lo mismo que \texttt{solver/labesat --symmetry}: la CNF intermedia es
idéntica, la prueba continúa el prefijo de satsuma y, si satsuma no se puede
aplicar, se resuelve la fórmula original con una prueba DRAT pura. Los topes se
fijan con las mismas variables de entorno (sección~\ref{sec:entorno}).
...
\begin{lstlisting}
solver/labesat [--symmetry | --no-symmetry] [opciones de kissat] <cnf> [<prueba>]
\end{lstlisting}
...
\begin{description}
  ...
  \item[\texttt{--symmetry}] Activa la ruptura de simetrías (satsuma). Hoy está
        \textbf{desactivada por defecto}: el experimento EXP-007 mostró que gana
        mucho en instancias con simetría explotable (37 de 45 resueltas frente a
        8), pero ralentiza 1{,}42$\times$ las que no la tienen. Se activará sola
        cuando la activación condicional (B3) esté validada. También se puede
        activar con \texttt{LABESAT\_SYMMETRY=1}.
  \item[\texttt{--no-symmetry}] Desactiva satsuma aunque \texttt{LABESAT\_SYMMETRY=1}.
...
\subsection{Opciones experimentales de LabeSAT}
Cambian la búsqueda y están \textbf{desactivadas por defecto} hasta que un
experimento preregistrado las valide. Se pasan a kissat como cualquier otra
opción.
\begin{description}
  \item[\texttt{--vivifyactivity=1}] \emph{Vivificación programada por
        actividad} (VSA, de Kissat-VSA, SAT Competition 2025). En modo estable,
        vivifica antes las cláusulas cuya variable menos activa tiene mayor
        puntuación. Con \texttt{0}, la búsqueda es idéntica a la de Kissat
        4.0.4. En evaluación: EXP-015.
  \item[\texttt{--modeadaptive=1}] Reparto adaptativo del presupuesto entre
        los modos estable y enfocado (A4.1). Cerrado sin efecto detectado
        (EXP-006); se conserva para reproducir aquel experimento.
\end{description}
...
# Resolver con ruptura de simetrías, límite de 5000 s, y escribir la prueba
solver/labesat --symmetry --time=5000 formula.cnf.xz prueba.out

# Solo kissat (comportamiento por defecto)
solver/labesat formula.cnf
\end{lstlisting}
...
\subsection{Variables de entorno}\label{sec:entorno}
\begin{center}
...
\texttt{LABESAT\_SYMMETRY} & 1 activa la ruptura de simetrías & 0 \\
\bottomrule
\end{tabularx}
\end{center}
...
\end{lstlisting}
Si no se usó \texttt{--symmetry}, o si satsuma no llegó a actuar, la prueba es
...
python3 scripts/run_ab_interleaved.py --solver solver/labesat \
    ...
    --opts-a="--no-symmetry" --opts-b="--symmetry" --timeout 180 --seeds 1
